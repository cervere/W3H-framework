...
def setCues(moment, x, z, close_entities, far_entities):
    ...
    for cent in close_entities:
        if cent["name"] == AGENT_NAME : continue
...
#        if   zdist <= REACH_SCOPE :
        moment['observation']['reach'].append(cent)
...
def prepareMoment(moment, ob):
    ...
    if "far_entities" in ob: #Make sure these keys like "far_entities" are picked from a common place even in the XML
        ...
        setCues(moment, x, z, ob.get(u'close_entities', []), ob.get(u'far_entities', []))
